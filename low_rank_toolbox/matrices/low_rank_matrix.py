...
Author: Benjamin Carrel, University of Geneva, 2022-2025
"""
...
class LowRankMatrix(LinearOperator):
    ...
    def __getitem__(self, key):
        """Access matrix elements, rows, columns, or sub-blocks efficiently.

        Slicing is factor-aware: row slicing touches ``_matrices[0]`` only,
        column slicing touches ``_matrices[-1]`` only. Cost is O(r^2 |rows|)
        or O(r^2 |cols|), never O(n^2 r).

        Returns
        -------
        - Python scalar for ``X[int, int]``.
        - 1-D ndarray for ``X[int, :]`` / ``X[:, int]`` / equal-length
          fancy-pair ``X[rows_arr, cols_arr]``.
        - 2-D ndarray for ``X[np.ix_(rows, cols)]``.
        - ``LowRankMatrix`` for ``X[rows_or_slice, cols_or_slice]`` when both
          axes are non-scalar (the factorisation is preserved).

        Notes
        -----
        Intermediate factors are shared by reference (no copy). Chained
        slicing (``X[rows, :][:, cols]``) does O(r^2 max(|rows|, |cols|))
        work and is fully supported.
        """
        if not isinstance(key, tuple):
            key = (key, slice(None))
        if len(key) != 2:
            raise IndexError("LowRankMatrix indexing requires exactly 2 indices.")
        row_idx, col_idx = key

        if row_idx is None or col_idx is None:
            raise IndexError(
                "LowRankMatrix does not support np.newaxis / None in indexing."
            )

        row_is_scalar = isinstance(row_idx, (int, np.integer))
        col_is_scalar = isinstance(col_idx, (int, np.integer))
        ...
        row_idx_n = _as_index(row_idx)
        col_idx_n = _as_index(col_idx)

        row_is_array = isinstance(row_idx_n, np.ndarray)
        col_is_array = isinstance(col_idx_n, np.ndarray)

        if (isinstance(row_idx_n, slice) and row_idx_n == slice(None)
                and isinstance(col_idx_n, slice) and col_idx_n == slice(None)):
            return LowRankMatrix(*self._matrices, _skip_memory_check=True)

        if (row_is_array and col_is_array
                and row_idx_n.ndim == 2 and col_idx_n.ndim == 2):
            return self.get_block(row_idx_n[:, 0], col_idx_n[0, :])

        if (row_is_array and col_is_array
                and row_idx_n.ndim == 1 and col_idx_n.ndim == 1
                and row_idx_n.shape == col_idx_n.shape):
            return self._gather_pair(row_idx_n, col_idx_n)

        if row_is_scalar:
            row_of_M0 = self._matrices[0][row_idx:row_idx + 1, :]
            tail = self._matrices[1:]
            if len(tail) == 1:
                row_vec = (row_of_M0 @ tail[0])[0]
            else:
                row_vec = np.linalg.multi_dot([row_of_M0, *tail])[0]
            if isinstance(col_idx_n, slice) and col_idx_n == slice(None):
                return row_vec
            return row_vec[col_idx_n]
        if col_is_scalar:
            col_of_ML = self._matrices[-1][:, col_idx:col_idx + 1]
            head = self._matrices[:-1]
            if len(head) == 1:
                col_vec = (head[0] @ col_of_ML)[:, 0]
            else:
                col_vec = np.linalg.multi_dot([*head, col_of_ML])[:, 0]
            if isinstance(row_idx_n, slice) and row_idx_n == slice(None):
                return col_vec
            return col_vec[row_idx_n]

        new_first = self._matrices[0][row_idx_n, :]
        new_last = self._matrices[-1][:, col_idx_n]
        if self.length == 2:
            new_factors = [new_first, new_last]
        else:
            new_factors = [new_first, *self._matrices[1:-1], new_last]
        return LowRankMatrix(*new_factors, _skip_memory_check=True)

    def _gather_pair(self, rows: ndarray, cols: ndarray) -> ndarray:
        """Zip-pair fancy indexing: 1-D array with ``X[rows[i], cols[i]]``.

        Cost O(r^2 |rows|), matches numpy semantics for equal-length 1-D
        fancy-index pairs.
        """
        L = self._matrices[0][rows, :]
        R = self._matrices[-1][:, cols]
        mid = L
        for M in self._matrices[1:-1]:
            mid = mid @ M
        return np.einsum("ij,ji->i", mid, R)

    def get_block(self, rows, cols) -> ndarray:
        """Extract a dense ``(|rows|, |cols|)`` block using the factorisation.

        Cost O(r^2 max(|rows|, |cols|)) — no n x n temporary is formed.
        """
        L = self._matrices[0][rows, :]
        R = self._matrices[-1][:, cols]
        if self.length == 2:
            return L @ R
        return np.linalg.multi_dot([L, *self._matrices[1:-1], R])
