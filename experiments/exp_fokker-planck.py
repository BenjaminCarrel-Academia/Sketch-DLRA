"""
Polished error-over-time figure for sketch DLRA on Fokker-Planck (2D, constant energy).
...
from matrix_ode_toolbox.problems import make_fokker_planck_2D_constant_energy
...
# Problem setup
ode, X0 = make_fokker_planck_2D_constant_energy(size)
...
def run_one_seed(method_key, seed):
    ...
    for i in range(len(t_eval) - 1):
        try:
            ...
            err = rel_err(Y.todense(), Xs_ref[i + 1])
        except Exception:
            break
...
# Plot
figures_dir = os.path.join(_here, 'figures', 'fokker-planck')
...
panels = [
    (ax_ortho, f'Fokker{EN}Planck {EM} orthogonal sketch',
        ('ortho_sketch_KSL', 'ortho_sketch_bug')),
    (ax_obl,   f'Fokker{EN}Planck {EM} oblique sketch',
...
png = os.path.join(figures_dir, 'fokker_planck_error.png')
pdf = os.path.join(figures_dir, 'fokker_planck_error.pdf')
