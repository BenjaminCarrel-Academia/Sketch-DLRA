...
class MatrixOdeSolution:
    ...
    def shape(self) -> tuple:
        "Shape of the solution matrices."
        return self.X0.shape

    def __getitem__(self, key):
        "Return the solution at integer index or time value."
        if isinstance(key, (int, np.integer)):
            return self.Xs[key]
        return self.Xs[self.ts == key][0]
    ...
    def visualise(self, meshgrid: tuple = None, **kwargs):
        ...
        """
        raise NotImplementedError('Not implemented yet. Implement it if necessary. You may want to add a plot function to the class MatrixOde, since the plot is specific to the ODE.')
