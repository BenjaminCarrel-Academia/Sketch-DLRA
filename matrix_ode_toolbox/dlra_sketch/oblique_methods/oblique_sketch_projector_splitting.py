"""
Oblique Sketch Projector Splitting method for sketch DLRA.
...
from matrix_ode_toolbox.dlra_sketch.substeps import (
    ...
    _make_minus_S_sketch_oblique_rhs,
)


class ObliqueSketchProjectorSplitting(SketchDlraSolver):
    """Oblique Sketch Projector Splitting (sKSL) integrator.

    Uses randomized Gram-Schmidt for basis extraction and oblique sketch
    projection (Theta P)^{-1}Theta in the K, -S, and L substep ODEs.

    Supports order 1 (Lie-Trotter) and order 2 (Strang) splitting.
    """

    name = 'Oblique Sketch Projector Splitting'
    ...
    def sKSL1(self, t_subspan: tuple, Y0: SketchSVD) -> SketchSVD:
        ...
        # (-S)-STEP
        mS_rhs = _make_minus_S_sketch_oblique_rhs(problem, P1, W0, Theta, Omega)
        S0_tilde = solve_matrix_ivp(SubstepProblem(mS_rhs, S1_hat.shape, 'minus_S_oblique'), t_subspan, S1_hat, **self.substep_kwargs)

        # L-STEP
        L0 = W0.dot(S0_tilde.T.conj())
        L_rhs = _make_L_sketch_oblique_rhs(problem, P1, Theta)
    ...
    def sKSL2(self, t_subspan: tuple, Y0: SketchSVD) -> SketchSVD:
        ...
        K0 = P0.dot(S0)
        K_rhs = _make_K_sketch_oblique_rhs(problem, W0, Omega)
        K_half = solve_matrix_ivp(SubstepProblem(K_rhs, K0.shape, 'K_oblique'), (t0, t_half), K0, **self.substep_kwargs)
        ...
        mS_rhs = _make_minus_S_sketch_oblique_rhs(problem, P_half, W0, Theta, Omega)
        S_half = solve_matrix_ivp(SubstepProblem(mS_rhs, S_half_hat.shape, 'minus_S_oblique'), (t0, t_half), S_half_hat, **self.substep_kwargs)

        # 3. Full L-step (t0 -> t1)
        L0 = W0.dot(S_half.T.conj())
        L_rhs = _make_L_sketch_oblique_rhs(problem, P_half, Theta)
        L_full = solve_matrix_ivp(SubstepProblem(L_rhs, L0.shape, 'L_oblique'), (t0, t1), L0, **self.substep_kwargs)
        ...
        mS_rhs2 = _make_minus_S_sketch_oblique_rhs(problem, P_half, W1, Theta, Omega)
        S1_half = solve_matrix_ivp(SubstepProblem(mS_rhs2, S1.shape, 'minus_S_oblique'), (t_half, t1), S1, **self.substep_kwargs)

        # 5. Half K-step (t_half -> t1)
        K1 = P_half.dot(S1_half)
        K_rhs2 = _make_K_sketch_oblique_rhs(problem, W1, Omega)
        K1_final = solve_matrix_ivp(SubstepProblem(K_rhs2, K1.shape, 'K_oblique'), (t_half, t1), K1, **self.substep_kwargs)
